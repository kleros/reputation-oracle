...
| Systemd sandboxing directives | `ProtectSystem=strict`, `PrivateTmp=true`, `NoNewPrivileges=true` — baseline for any systemd service | LOW | All available on Ubuntu 24.04 (systemd 255.4); add `MemoryMax=512M`, `CPUQuota=50%` as resource ceiling |

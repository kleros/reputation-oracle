...
1. **Locate the .gsd/ directory** — check the current working directory (or `--path` argument):
   ```bash
   node "/Users/jaybuidl/project/kleros/reputation-oracle/.claude/get-shit-done/bin/gsd-tools.cjs" from-gsd2 --dry-run
   ```
...
3. **Run the migration** after confirmation:
   ```bash
   node "/Users/jaybuidl/project/kleros/reputation-oracle/.claude/get-shit-done/bin/gsd-tools.cjs" from-gsd2
   ```
